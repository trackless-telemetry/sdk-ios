...
/// In-memory per-session first-occurrence tracker for error events.
///
/// Records which normalized error names have occurred at least once in the current
/// session, so the first `error(name)` call in a session can be marked with
/// `firstOccurrences: 1` (feeding server-side session-reach counts for errors) and
/// repeats are not. Dedup is keyed on the normalized error **name only**, not
/// `(name, severity, code)` — a session that hits the same error at several
/// severities or with several codes still contributes exactly one first occurrence,
/// mirroring the name-only dedup `FeatureFirstUseTracker` applies to `(name, detail)`.
///
